...
module.exports = class HolepunchRouter {
  ...
  async peerHandshake (target, { noise, peerAddress, relayAddress, socket }, to) {
    const dht = this.dht
    ...
    const res = await dht.request({ command: COMMANDS.PEER_HANDSHAKE, target, value: requestValue }, to, { socket })
