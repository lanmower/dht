  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress }) {
    ...
    if (error === ERROR.NONE) {
      ...
      hs.onsocket = (socket, port, host) => {
        ...
        hs.rawStream.connect(socket, remotePayload.streamId, port, host)
    ...
    try {
      hs.reply = await handshake.send({
        ...
        protocols: this._protocols,
        streamId: hs.rawStream ? hs.rawStream.id : 0,
    ...
    if (remotePayload.firewall === FIREWALL.OPEN) {
      const socket = this.dht.socket
      hs.onsocket(socket, clientAddress.port, clientAddress.host)
      return hs
    }
