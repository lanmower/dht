...
exports.noisePayload = {
  preencode (state, m) {
    ...
    c.uint.preencode(state, m.streamId)
  ...
  encode (state, m) {
    ...
    c.uint.encode(state, m.streamId)
  ...
  decode (state) {
    ...
    if (version !== 1) { // do not attempt to decode but return this back to the user so they can actually handle it
      return { version, error: 0, firewall: 0, protocols: 0, streamId: 0, holepunch: null, addresses: null }
    }
