...
async function holepunch (c, opts) {
  ...
  if (payload.firewall === FIREWALL.OPEN || (relayed && !remoteHolepunchable)) {
    if (payload.addresses && payload.addresses.length) {
      ...
      c.onsocket(socket, payload.addresses[0].port, payload.addresses[0].host)
      return
    }
  ...
  if (relayed && payload.addresses && clientAddress.host === serverAddress.host) {
    for (const addr of payload.addresses) {
      if (isPrivate(addr.host)) {
        const socket = c.dht.socket
...
async function connectThroughNode (c, address, socket) {
  ...
  c.onsocket = function (socket, port, host) {
    ...
    c.rawStream.connect(socket, c.connect.payload.streamId, port, host)
