...
class Pair {
  ...
  connect (addrs, from, to, relayed) {
    ...
    const maybeLocal = sameNetwork && localAddress && relayed

    if (maybeLocal) {
      if (this.handshake.isInitiator) this._connect(localAddress)
      // Fallthrough and the others as well
    }
    ...
    if (this.remoteFirewall === FIREWALL.OPEN && !maybeLocal) {
